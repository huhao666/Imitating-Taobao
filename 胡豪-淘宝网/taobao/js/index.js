	...
	return {
		firstlun: (function() {		
			...
			if(getCookie('UserName')){
				$('.cover-login1').hide();
				$('.cover-login2').show().find('span').html('你好,'+getCookie('UserName'));
			}
			$('.cover-login2 a').on('click',function(){
			...
			//导航栏下拉列表
			$(".nav-li1").on("mouseover",function () {
				$(".navList").show();
			});
			$(".nav-li1").on("mouseout",function () {
				$(".navList").hide();
			});
			...
			//search栏选择
			$(".search-t ul li").on('click',function(){
				$(this).addClass('selected').siblings().removeClass('selected');
			})
			
			//隐藏二维码
			$('#qr_c').on('click',function(){
				$(".qr").hide();
			})
...
$(window).scroll(function() {
	...
	if(top >= 380) {
		$('.follow').css({ 'position': 'fixed', 'top': 0, 'z-index': 23, 'display': 'block' })
	} else {
